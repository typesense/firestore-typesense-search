...
module.exports = functions.firestore.document(config.firestoreCollectionPath)
    .onWrite(async (snapshot, context) => {
      ...
      if (snapshot.after.data() == null) {
        ...
        functions.logger.debug(`[V1 API]: Deleting document ${documentId}`);
        ...
        if (config.shouldLogTypesenseInserts) {
          functions.logger.debug(`[V1 API]: Upserting document ${JSON.stringify(typesenseDocument)}`);
        } else {
          functions.logger.debug(`[V1 API]: Upserting document ${typesenseDocument.id}`);
        }
        return await typesense
            .collections(encodeURIComponent(config.typesenseCollectionName))
            .documents()
